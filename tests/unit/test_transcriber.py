...
def test_auto_detect_picks_cuda_when_device_count_positive():
    with patch("dictate.transcriber.WhisperModel") as WM, \
         patch("dictate.transcriber._cuda_available", return_value=True):
...
def test_auto_detect_falls_back_to_cpu_when_no_cuda():
    with patch("dictate.transcriber.WhisperModel") as WM, \
         patch("dictate.transcriber._cuda_available", return_value=False):
        t = Transcriber(size="small", device="auto", compute_type="auto")
    assert t.device == "cpu"
...
def test_cuda_init_failure_falls_back_to_cpu_with_last_error():
    ...
    with patch("dictate.transcriber.WhisperModel", side_effect=fake_wm):
...
def test_cpu_init_failure_raises():
    with patch("dictate.transcriber.WhisperModel", side_effect=RuntimeError("boom")):
